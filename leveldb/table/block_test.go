...
type onetimeBlockTesting struct {
	tr *LightReader
	b  *block
}

func (t *onetimeBlockTesting) TestNewIterator(slice *util.Range) iterator.Iterator {
	return &onetimeBlockIter{iter: t.tr.reader.newBlockIter(t.b, nil, slice, false)}
}
...
var _ = testutil.Defer(func() {
			...
			for restartInterval := 1; restartInterval <= 5; restartInterval++ {
				Describe(fmt.Sprintf("with restart interval of %d", restartInterval), func() {
					...
					Test := func() {
						...
						// Do testing.
						testutil.KeyValueTesting(nil, kv.Clone(), br, nil, nil, false)
					}
	...
	Describe("OneTimeBlock", func() {
		Build := func(kv *testutil.KeyValue, restartInterval int) *onetimeBlockTesting {
			// Building the block.
			bw := &blockWriter{
				restartInterval: restartInterval,
				scratch:         make([]byte, 30),
			...
			return &onetimeBlockTesting{
				tr: &LightReader{
					reader: &Reader{cmp: comparer.DefaultComparer},
					min:    min,
					max:    max,
				},
				b: &block{
			...
			for restartInterval := 1; restartInterval <= 5; restartInterval++ {
				Describe(fmt.Sprintf("with restart interval of %d", restartInterval), func() {
					...
					Test := func() {
						...
						br := Build(kv, restartInterval)
						// Do testing.
						testutil.KeyValueTesting(nil, kv.Clone(), br, nil, nil, true)
					}
				...
				Describe(fmt.Sprintf("with restart interval of %d", restartInterval), func() {
					...
					Test := func(r *util.Range) func(done Done) {
						return func(done Done) {
							...
							testutil.DoOneTimeIteratorTesting(&t)
							iter.Release()
							done <- true
						}
					}
