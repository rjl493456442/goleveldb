...
// is it's more resource efficient. The resource won't be allocated only if
// the first entry in the iterator is accessed. In another word, allocate
// the resource in the lazy way.
...
func (i *lightIndexIter) initialize() error {
	...
	i.iter = i.tr.reader.newBlockIter(indexBlock, rel, i.slice, true)
	return nil
}
...
func (i *lightIndexIter) Get() iterator.Iterator {
	...
	if i.slice != nil && (i.iter.isFirst() || i.iter.isLast()) {
		slice = i.slice
	}
...
func NewLightReader(f io.ReaderAt, size int64, min, max []byte, fd storage.FileDesc, cache *cache.NamespaceGetter, bpool *util.BufferPool, o *opt.Options) (*LightReader, error) {
	reader, err := NewReader(f, size, fd, cache, bpool, o)
	if err != nil {
		return nil, err
	}
	// todo(rjl493456442) the meta will be loaded if the cache is nil
	return &LightReader{
		reader: reader,
		min:    append([]byte{}, min...),
		max:    append([]byte{}, max...),
...
// ToReader converts the light reader to the normal reader.
func (r *LightReader) ToReader() *Reader {
	return r.reader
}
