...
type lightTableWrapper struct {
	*LightReader
}

func (t lightTableWrapper) TestFind(key []byte) (rkey, rvalue []byte, err error) {
	return t.LightReader.Find(key, false, nil)
}

func (t lightTableWrapper) TestGet(key []byte) (value []byte, err error) {
	return t.LightReader.Get(key, nil)
}

func (t lightTableWrapper) TestNewIterator(slice *util.Range) iterator.Iterator {
	return t.LightReader.NewIterator(slice, nil)
}
	...
	Describe("Table", func() {
		Describe("approximate offset test", func() {
			...
			It("Should be able to approximate offset of a key correctly", func() {
				...
				tr, err := NewReader(bytes.NewReader(buf.Bytes()), int64(buf.Len()), storage.FileDesc{}, nil, nil, o, true)
				Expect(err).ShouldNot(HaveOccurred())
		...
		Describe("read test", func() {
			Build := func(kv testutil.KeyValue) testutil.DB {
				...
				// Opening the table.
				tr, _ := NewReader(bytes.NewReader(buf.Bytes()), int64(buf.Len()), storage.FileDesc{}, nil, nil, o, true)
			...
			Test := func(kv *testutil.KeyValue, body func(r *Reader)) func() {
				return func() {
					...
					testutil.KeyValueTesting(nil, *kv, db, nil, nil, false)
				}
			}

			testutil.AllKeyValueTesting(nil, Build, nil, nil, false)
	...
	Describe("LightTable", func() {
		Describe("approximate offset test", func() {
			...
			It("Should be able to approximate offset of a key correctly", func() {
				...
				tr, err := NewLightReader(bytes.NewReader(buf.Bytes()), int64(buf.Len()), []byte("k01"), []byte("k07"), storage.FileDesc{}, nil, nil, o)
				Expect(err).ShouldNot(HaveOccurred())
				CheckOffset := func(key string, expect, threshold int) {
					offset, err := tr.OffsetOf([]byte(key))
					Expect(err).ShouldNot(HaveOccurred())
		...
		Describe("light read test", func() {
			Build := func(kv testutil.KeyValue) testutil.DB {
				...
				if kv.Len() > 0 {
					...
					key, _ = kv.Index(kv.Len() - 1)
					max = append([]byte{}, key...)
				}

				// Opening the table.
				tr, _ := NewLightReader(bytes.NewReader(buf.Bytes()), int64(buf.Len()), min, max, storage.FileDesc{}, nil, nil, o)
				return lightTableWrapper{tr}
			}
			//Test := func(kv *testutil.KeyValue, body func(r *LightReader)) func() {
			//	return func() {
			//		db := Build(*kv)
			//		if body != nil {
			//			body(db.(lightTableWrapper).LightReader)
			//		}
			//		testutil.KeyValueTesting(nil, *kv, db, nil, nil, false)
			//	}
			//}

			testutil.AllKeyValueTesting(nil, Build, nil, nil, true)
			//Describe("with one key per block", Test(testutil.KeyValue_Generate(nil, 9, 1, 1, 10, 512, 512), func(r *LightReader) {
			//	It("should have correct blocks number", func() {
			//		indexBlock, err := r.readBlock(r.indexBH, true)
			//		Expect(err).To(BeNil())
			//		Expect(indexBlock.restartsLen).Should(Equal(9))
